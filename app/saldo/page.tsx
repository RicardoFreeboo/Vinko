...
export default async function Saldo() {
  ...
  const [{ data: p }, { data: cfgRows }] = await Promise.all([
    sb!.from("profiles")
      .select("points, xp, marcador_total, streak_days, streak_best, streak_last, streak_shields, streak_broken_days, streak_recover_until, division, daily_bonus_last, daily_bonus_step, role")
      .eq("id", session.id)
      .maybeSingle(),
  ...
  return (
    <main className="amb mx-auto flex min-h-dvh w-full max-w-[430px] flex-col gap-5 px-5 pb-24 pt-6">
      ...
      <section className="flex flex-col gap-2">
        ...
        <EarnRow icon="🤝" title={t("vinkos.earn.invite")} sub={t("vinkos.earn.inviteSub", { m: String(eco.invitee_pts) })} value={`+${eco.invite_pts}`} href="/saldo" />
        ...
        <EarnRow icon="🎁" title={t("vinkos.earn.daily")} sub={t("vinkos.earn.dailySub", { total: String(eco.daily_bonus.length) })} value={`${ladderMin}→${ladderMax}`} href="/saldo" />
        <EarnRow icon="🔥" title={t("vinkos.earn.streak")} sub={t("vinkos.earn.streakSub")} value={`+${eco.streak7_pts}`} href="/feed" />
        {isAdult && (
          <EarnRow icon="📺" title={t("vinkos.earn.ad")} sub={t("vinkos.earn.adSub", { cap: String(eco.ad_reward_daily_cap) })} value={`+${eco.ad_reward_pts}`} href="/saldo" />
        )}
